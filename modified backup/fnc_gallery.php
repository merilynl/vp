<?php
	...
	function read_public_photo_page($privacy, $limit){
        $photo_html = null;
        $conn = new mysqli($GLOBALS["server_host"], $GLOBALS["server_user_name"], $GLOBALS["server_password"], $GLOBALS["database"]);
		$conn->set_charset("utf8");
        //$stmt = $conn->prepare("SELECT filename, alttext FROM vp_photos_2 WHERE privacy >= ? AND deleted IS NULL");
	...
	function read_own_photo_thumbs($page, $limit){
		...
		//LIMIT x - mitu näidata
		//LIMIT x,y - mitu vahele jätta, mitu näidata
		$stmt = $conn->prepare("SELECT id, filename, alttext, privacy FROM vp_photos_2 WHERE userid = ? AND deleted IS NULL ORDER BY vp_photos_2.id DESC LIMIT ?,?");
        echo $conn->error;
